...
export default function ProductCard({product, showTitle = true, showPrice = true, className}: IProductCard) {
    const {pickUpPrice} = useCalculatePrice(product)

    return (
    <Link href={`/product-list-poc/${product.slug}`} style={{gridArea: product.slug }} className={className}>
      ...
      <CardContent className="w-[100%] h-[100%] p-0">
        <Image src={product.images[0]} alt="product image" className="w-[100%] h-[100%] object-cover  hover:scale-110 duration-[0.5s]" width={800} height={800}/>
...
:
        ...
        <small>from:</small>
    <span className="text-2xl font-bold bg-cyan-950 bg-opacity-75 rounded py-1 px-3">£{pickUpPrice}</span>
